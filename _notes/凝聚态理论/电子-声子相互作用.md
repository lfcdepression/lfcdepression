...
这个方程有很明显的物理意义，描述了电子声子相互作用的两个过程。(a)首先$k$电子吸收$q$声子变为$k+q$态上的电子；(b)$k$电子发射$q$声子变为$k-q$态上的电子。当$k$在费米球外而$k-q$在费米球内时$ a_{q}^{\dagger} C^{\dagger}_{k-q}C_{k} $将代表消灭$k$ 电子和$k-q$ 空穴，并产生$q$ 声子，即常见的电子-空穴对复合发射声子的过程。类似的$ a_{q}C_{k+q}^{\dagger}C_{k} $ 也可以代表吸收声子后产生电子-空穴对。
