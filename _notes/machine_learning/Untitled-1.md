...
&=-\frac1{2t}\operatorname{E}[x_0-x_t\mid x_t]
\end{aligned}
\end{equation}$$
...
&=x_t+\frac12(\mathbb{E}[x_{t-\Delta t}\mid x_t]-x_t)
\end{aligned}
\end{equation}$$
...
- DDPm类型情况下从$p(x_0\mid x_1)$采样得到样本，如果正向过程充分混合$\sigma_q$很大，那么终点$x_1$将几乎独立于初始点。因此$p(x_0\mid x_1) \approx p(x_0)$，理想的DDPM输出完全不依赖于起始点$x_1$，相反DDIM是确定性的，因此对于固定的$x_1$会产生固定值，依赖于$x_1$。
